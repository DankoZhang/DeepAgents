...
def load_mcp_tools(tool_def: ToolDefinition) -> list[Any]:
    ...
    with _mcp_tools_lock:
        ...
        if hit is not None and hit[0] == fingerprint:
            return list(hit[1])
    try:
        tools = _run_coro(_aload_mcp_tools(tool_def))
    except Exception:
...
def interrupt_tool_names_from_payloads(payloads: list[dict[str, Any]]) -> dict[str, bool]:
    """
    ...
    for payload in payloads:
        ...
        if tool_type == "mcp":
            ...
            if include:
                ...
                continue
            try:
                expanded = expand_tool_definition(tool_definition_from_snapshot(payload))
            except Exception:  # noqa: BLE001
                logger.exception(
